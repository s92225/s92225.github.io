...
版本：0.8（本機 MVP 0.4 關卡制更新） · 更新日期：2026-09-22  
狀態：30 關可玩網頁 MVP 已實作並驗證；正式 iOS／Android App 尚未開發。  
「夾夾島」為工作名稱，尚未由用戶確認正式品牌名稱。

> **最新產品決定（MVP 0.4）**：本文件早期的 B「3D 遊戲＋獨立實物商店」已被取代。產品現為純數碼關卡制夾爪遊戲；不提供實物獎勵、實物兌換、實物購買或線下寄送。以下第 2–20 節保留部分歷史記錄時，以第 21 節為準。
...
## 16. 已確認的下一階段：可玩網頁 MVP

2026-09-22：用戶明確指示「do MVP」，採用先做可玩網頁版本、在 in-app browser 試玩及留下修改意見的工作方式。
...
製作狀態：進行中。現有 App-Preview.html 仍為靜態設計預覽，不能當成已完成的可玩 MVP。實際技術選擇、物理近似方式、測試結果及啟動網址會在實作完成後補充。

第 12 節的早期製作次序，以本節最新決定為準；Unity 仍只是正式 App 的候選引擎，不限制網頁 MVP 的技術選擇。

## 17. 本機 MVP 0.1 實作紀錄（2026-09-22，歷史）
...
- MVP 以 30 個資料驅動關卡驗證擴展方式，涵蓋角色、糖果、禮盒及 26 款不同日用品。長期可以同一資料格式擴充至大量關卡。
...
仍待產品驗證：正式金幣售價與組合、生命／恢復節奏、錦囊長期平衡、數百／數千關的內容生產方式、登入／伺服器／防作弊，以及正式原生 App 技術。MVP 0.4 沒有修改 Figma、接入支付或公開發布。

## 22. MVP 0.4.1：開頂機身、防穿透及第二關難度修正
...
## 30. MVP 0.5.4：多件計分、持續結算動畫及完整寶物圖案
...
## 32. MVP 0.5.6：大型獎勵演出、放大星星糖及滑鼠重製
